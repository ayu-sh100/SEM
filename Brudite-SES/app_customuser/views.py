...
def Register(request):
            ...
            if serializer.is_valid():
                    ...
                    htmlfile = render_to_string("Register.html",userdetails)
                    
                    # Set up the email message
                    subject, from_email, to = f"""Registration Successful {user.username}""", settings.EMAIL_HOST_USER, user.email
        ...
        try: 
            ...
            if user is not None:
                ...
                if user:
                    ...
                    htmlfile = render_to_string("Login.html",userdetails)
                    
                    # Set up the email message
                    subject, from_email, to = "Security Alert", settings.EMAIL_HOST_USER, user.email
                    text_content = "This is an important message."
                    html_content = htmlfile
                    msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
                    msg.attach_alternative(html_content, "text/html")
